...
void simil(unsigned char *S, int n, unsigned char *a, int nb, int k) {
    for (int i = 0; i < n; i++) {
        int posicaomax = i;
        int similaridademax = card_inters(a, S + i, nb);

        for (int j = i + 1; j < n; j++) {
            int similarity = card_inters(a, S + j, nb);

            if (similarity > similaridademax) {
                posicaomax = j;
                similaridademax = similarity;
            }
        }
        ...
        if (posicaomax != i) {
            ...
            memcpy(temp, S + i, nb);
            memcpy(S + i, S + max_index, nb);
            memcpy(S + max_index, temp, nb);
        }
    }

    // Reduz o tamanho do conjunto S para k elementos, se necessário
    if (k < n) {
        n = k;
    }
}
